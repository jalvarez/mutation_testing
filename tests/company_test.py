...
def test_default_raise(company):
    ...
    company.add_employee(Employee("123", "Tom", original_salary))

    company.everybody_gets_raise_by()

    employee = company.find_employee_by_id("123")
    assert new_salary == pytest.approx(employee.get_salary())
...
def test_employee_with_largest_salary_with_same(company):
    company.add_employee(Employee("003", "Carl", 110_000.00))
    company.add_employee(Employee("001", "Alice", 110_000.00))

    highest_earner = company.employee_with_largest_salary()

    assert "Carl" == highest_earner.get_name()


def test_employee_with_lowest_salary_with_same(company):
    company.add_employee(Employee("003", "Carl", 110_000.00))
    company.add_employee(Employee("001", "Alice", 110_000.00))

    highest_earner = company.employee_with_lowest_salary()
